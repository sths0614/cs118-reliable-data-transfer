...
int main(int argc, char* argv[]) 
{
	...
	sockfd = rdt_socket(pCorrupt, pLoss, winSize);
	if (sockfd < 0)
	...
	// rdt_recvfrom(sockfd, &fname, (struct sockaddr*) &cli_addr, &zero_len);
	...
	// Send the file!
	rdt_sendto(sockfd, fbuf, fsize, (struct sockaddr*) &cli_addr, sizeof(cli_addr));
